...
class Config(object):
    ...
    def every(self, key, default=None):
        """Return the value at the given key from each config module."""
        return [getattr(config, key, default) for config in self.configs]

    def last(self, key, default=None):
        """Return the value at the key from the last config defining it."""
    ...
    @staticmethod
    def load(name):
        """Import and returns a module from dotted form or filename."""
        if path.exists(name):
