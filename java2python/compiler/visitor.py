    ...
    def acceptVarDeclaration(self, node, memo):
        ...
        for varDecl in varDecls.childrenOfType(tokens.VAR_DECLARATOR):
            ...
            if node.firstChildOfType(tokens.MODIFIER_LIST):
                identExp.modifiers = [
                    child.text
                    for child in node.firstChildOfType(tokens.MODIFIER_LIST).children
                ]
...
class MethodContent(Base):
    ...
    def acceptCatch(self, node, memo):
        ...
        self.expr.right = self.factory.expr(
            fs=FS.l + " as " + FS.r, left=cname, right=cvar
        )
    ...
    def acceptFor(self, node, memo):
        ...
        if (
            not node.firstChildOfType(tokens.BLOCK_SCOPE)
            or not node.firstChildOfType(tokens.BLOCK_SCOPE).children
        ):
    ...
    goodReturnParents = (tokens.BLOCK_SCOPE,)
    ...
    def acceptCastExpr(self, node, memo):
        ...
        if typeName in tokens.primitiveTypeNames:
            ...
            if handler:
                ...
                warning(
                    "No handler to perform cast of non-primitive type %s.", typeName
                )
        self.left, self.right = visitors = factory(parent=self), factory(parent=self)
        self.zipWalk(node.children, visitors, memo)
        ...
        def acceptPrePost(self, node, memo):
            ...
            if node.withinExpr:
                ...
                if pre:
                    ...
                    left = rename
                    block.adopt(
                        factory(fs=FS.l + " = " + FS.r, left=rename, right=name)
                    )
