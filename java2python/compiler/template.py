...
class Base(object, metaclass=FactoryTypeDetector):
    ...
    * Serialization
    ...
    isAnnotation = (
        ...
        isComment
    ) = isEnum = isExpression = isInterface = isMethod = isModule = isStatement = False
    ...
    def adopt(self, child, index=-1):
        """Add child to this objects children and sets the childs parent."""
    ...
    def altIdent(self, name):
        ...
        for klass in self.parents(lambda v: v.isClass):
            if name in klass.variables:
                ...
                ## Below change make by VSK on Apr 16, 2020 so as to not generate any
                ## Python classmethods for ASKE
...
class Class(ClassMethodSharedMixin, Base):
    ...
    def iterBody(self):
        ...
        def sprinkleBlanks(body):
            ...
            for item in body:
                if prev:
                    if not isinstance(prev, type(item)) and not prev.isComment:
                        yield blank
